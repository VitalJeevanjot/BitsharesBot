...
var _btsdexSerializer = require("btsdex-serializer");
...
class TransactionBuilder {
  ...
  process_transaction(cwallet, signer_pubkeys = null, broadcast = false) {
    ...
    if ((0, _btsdexApi.getConfig)().chainId !== wallet_object.get("chain_id")) return Promise.reject("Mismatched chain_id; expecting " + wallet_object.get("chain_id") + ", but got " + (0, _btsdexApi.getConfig)().chainId);
    return this.set_required_fees().then(() => {
      ...
      return this.get_potential_signatures().then(({
        pubkeys,
        addys
      }) => {
  ...
  finalize() {
    return new Promise((resolve, reject) => {
      ...
      resolve(_btsdexApi.database.getObjects(["2.1.0"]).then(r => {
        head_block_time_string = r[0].time;
        if (this.expiration === 0) this.expiration = base_expiration_sec() + (0, _btsdexApi.getConfig)().expireInSecs;
        this.ref_block_num = r[0].head_block_number & 0xffff;
        this.ref_block_prefix = new _safeBuffer.Buffer(r[0].head_block_id, "hex").readUInt32LE(4); //DEBUG console.log("ref_block",@ref_block_num,@ref_block_prefix,r)
        ...
        this.tr_buffer = _btsdexSerializer.ops.transaction.toBuffer(this);
      }));
    });
  }
  ...
  id() {
    ...
    return _btsdexEcc.hash.sha256(this.tr_buffer).toString("hex").substring(0, 40);
  }
  /**
  ...
  get_type_operation(name, operation) {
    ...
    (0, _assert.default)(operation, "operation");
    var _type = _btsdexSerializer.ops[name];
    (0, _assert.default)(_type, `Unknown operation ${name}`);
    var operation_id = _btsdexSerializer.ChainTypes.operations[_type.operation_name];
    ...
    if (name === "proposal_create") {
      ...
      operation.expiration_time || (operation.expiration_time = base_expiration_sec() + (0, _btsdexApi.getConfig)().expireInSecsProposal);

      if (requiresReview) {
        operation.review_period_seconds = extraReview + Math.max(committee_min_review, 24 * 60 * 60 || (0, _btsdexApi.getConfig)().reviewInSecsCommittee);
        /*
  ...
  update_head_block() {
    return _btsdexApi.database.getObjects(["2.0.0", "2.1.0"]).then(function (res) {
      let [g, r] = res;
      head_block_time_string = r[0].time;
  ...
  set_required_fees(asset_id, removeDuplicates) {
    ...
    for (let i = 0, op; i < this.operations.length; i++) {
      ...
      let opObject = _btsdexSerializer.ops.operation.toObject(op);
    ...
    let promises = [];
    promises.push(Promise.all(feeAssets.map(id => {
      return _btsdexApi.database.getRequiredFees(operations, id);
    })).catch(err => {
    ...
    if (feeAssets.length > 1 || feeAssets[0] !== "1.3.0") {
      ...
      promises.push(_btsdexApi.database.getRequiredFees(operations, "1.3.0"));
      promises.push(_btsdexApi.database.getObjects(dynamicObjectIds));
    }
    ...
    return Promise.all(promises).then(results => {
      ...
      let feePoolMap = !!dynamicObjects ? dynamicObjects.reduce((map, object) => {
      ...
      for (let i = 0; i < operations.length; i++) {
        ...
        if (isProposal(op)) {
          ...
          op[1].proposed_ops.forEach((prop, y) => {
            ...
            if (!proposalFeeMap[i][propFeeAsset]) proposalFeeMap[i][propFeeAsset] = {
              total: 0,
              ops: [i],
              propIdx: []
            };
      ...
      let set_fee = (operation, opIndex) => {
        if (!operation.fee || operation.fee.amount === 0 || operation.fee.amount.toString && operation.fee.amount.toString() === "0" // Long
            ...
            if (removeDuplicates) {
              let op = _btsdexSerializer.ops.operation.toObject(this.operations[opIndex]);
  ...
  get_potential_signatures() {
    var tr_object = _btsdexSerializer.ops.signed_transaction.toObject(this);

    return Promise.all([_btsdexApi.database.getPotentialSignatures(tr_object), _btsdexApi.database.getPotentialAddressSignatures(tr_object)]).then(function (results) {
      return {
        pubkeys: results[0],
        addys: results[1]
      };
  ...
  get_required_signatures(available_keys) {
    ...
    var tr_object = _btsdexSerializer.ops.signed_transaction.toObject(this); //DEBUG console.log('... tr_object',tr_object)


    return _btsdexApi.database.getRequiredSignatures(tr_object, available_keys).then(function (required_public_keys) {
  ...
  add_signer(private_key, public_key = private_key.toPublicKey()) {
    ...
    if (!public_key.Q) {
      public_key = _btsdexEcc.PublicKey.fromPublicKeyString(public_key);
    } // prevent duplicates
  ...
  sign(chain_id = (0, _btsdexApi.getConfig)().chainId) {
    if (!this.tr_buffer) {
      throw new Error("not finalized");
    }
    ...
    for (var i = 0; 0 < end ? i < end : i > end; 0 < end ? i++ : i++) {
      ...
      var sig = _btsdexEcc.Signature.signBuffer(_safeBuffer.Buffer.concat([new _safeBuffer.Buffer(chain_id, "hex"), this.tr_buffer]), private_key, public_key);
  ...
  serialize() {
    return _btsdexSerializer.ops.signed_transaction.toObject(this);
  }
...
function _broadcast(was_broadcast_callback) {
  return new Promise((resolve, reject) => {
    ...
    var tr_object = _btsdexSerializer.ops.signed_transaction.toObject(this); // console.log('... broadcast_transaction_with_callback !!!')


    _btsdexApi.network.broadcastTransactionWithCallback(function (res) {
      return resolve(res);
      ...
      reject(new Error(message + "\n" + "bitshares-crypto " + " digest " + _btsdexEcc.hash.sha256(this.tr_buffer).toString("hex") + " transaction " + this.tr_buffer.toString("hex") + " " + JSON.stringify(tr_object)));
      return;
    });
